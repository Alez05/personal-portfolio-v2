    ...
    <section className="bp-section">
      <div className="bp-container">
        ...
        <div className="bp-right">
          ...
          {/* Desktop CTA */}
          {cta && (
            <a href={cta.link} className="bp-cta-button bp-cta-desktop">
              {cta.label}
            </a>
          )}
        </div>
        ...
        <div className="bp-left">
          <div className="bp-carousel">
            <div className="bp-carousel-track" style={{ transform: `translateX(-${index * 100}%)` }}>
              {blogs.map((blog, i) => (
                <a key={i} href={blog.link} className="bp-card">
                  <img src={blog.image} alt={blog.title ?? `Blog ${i + 1}`} className="bp-card-image" />
                  {blog.title && (
            ...
            {blogs.length > 1 && (
              <>
                <button className="bp-carousel-btn prev" onClick={prev}>‹</button>
                <button className="bp-carousel-btn next" onClick={next}>›</button>
              </>
            )}
          </div>
          {/* Mobile CTA */}
          {cta && (
            <a href={cta.link} className="bp-cta-button bp-cta-mobile">
              {cta.label}
            </a>
          )}
        </div>
